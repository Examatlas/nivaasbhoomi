...
async function main() {
  ...
  console.log(`\n${confirm ? "⚠ LIVE IMPORT" : "DRY RUN"} — seed listings from ${file}`);
  ...
  console.log(`Seed expiry: ${seedExpiresAt.toISOString().slice(0, 10)} (+${days} days).`);
  if (errors.length) {
    console.log("\nSkipped:");
  ...
  const res = await (await import("@/lib/db/models/Listing")).Listing.collection.insertMany(docs);
