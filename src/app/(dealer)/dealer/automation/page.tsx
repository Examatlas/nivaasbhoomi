...
import { DeveloperApiCard } from "@/components/dealer/developer-api-card";
    ...
    <DealerShell active="/dealer/automation">
      ...
      {connectedFlag && dealer.zenithConnected && (
        <div className="mb-5 flex items-center gap-2 rounded-card border border-success-100 bg-success-50 px-4 py-3 text-sm text-success-700">
          <CheckCircle2 className="size-4" /> Connected to Zenith Code. Your listings now
      ...
      {errorCode && (
        <div className="mb-5 flex items-start gap-2 rounded-card border border-danger-100 bg-danger-50 px-4 py-3 text-sm text-danger-700">
          ...
          <div>
            <p>
              {ZENITH_ERROR_MESSAGES[errorCode] ?? "Couldn't connect to Zenith Code."}
            </p>
            {(errorCode === "no_number" || errorCode === "no_org") && keys && (
              <p className="mt-1 text-meta">
                Dev hint — profile fields returned: {keys}.
              </p>
            )}
          </div>
        </div>
        ...
        {dealer.zenithConnected ? (
          <>
            ...
            <p className="mt-4 text-sm text-muted-foreground">
              Your listings show a{" "}
              <span className="font-medium text-ink-800">WhatsApp</span> button that
              routes buyers straight into your Zenith Code automation.
            </p>
            <div className="mt-5">
          ...
          <>
            <p className="mt-5 text-sm text-muted-foreground">
              Connect your Zenith-registered WhatsApp number. Once verified, your
              listings&apos; button switches from{" "}
              <span className="font-medium text-ink-800">Contact Us</span> to a{" "}
              <span className="font-medium text-ink-800">WhatsApp</span> button that
              routes buyers into your Zenith AI agent.
            </p>

            <ul className="mt-5 flex flex-col gap-2.5">
              <Step
                icon={MessageCircle}
                title="Authorize on Zenith Code"
                body="Sign in and approve NivaasBhoomi."
              />
              <Step
                icon={Bot}
                title="We link your WhatsApp number"
                body="We link the Zenith-registered WhatsApp number that will receive buyer enquiries — it may differ from your NivaasBhoomi contact number."
              />
              <Step
                icon={ArrowRight}
                title="Button switches to WhatsApp"
                body="Buyers reach your Zenith AI agent directly."
              />
            </ul>
      ...
      <p className="mt-4 text-meta text-muted-foreground">
        Zenith Code is a separate product. Connecting is optional — your leads keep
      ...
      <DeveloperApiCard
        apiBaseUrl={absoluteUrl("/api/agent")}
        hasKey={dealer.agentApiKey.hasKey}
        last4={dealer.agentApiKey.last4}
      />
    </DealerShell>
  );
}
